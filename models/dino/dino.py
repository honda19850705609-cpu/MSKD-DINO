...
from .utils import sigmoid_focal_loss, sigmoid_focal_loss_with_class_weights, MLP
...
class DINO(nn.Module):
    ...
    def __init__(self, backbone, transformer, num_classes, num_queries, 
        ...
        if num_feature_levels > 1:
            ...
            for _ in range(num_backbone_outs):
                ...
                # Optional: P2 is huge; allow stride=2 projection to reduce memory while keeping layer1 semantics.
                if _ == 0 and int(p2_proj_stride) == 2:
        ...
        # Optional: BiFPN feature fusion after projection (helps small objects / dense scenes).
        self.use_bifpn_neck = bool(use_bifpn_neck)
        if self.use_bifpn_neck:
            from .bifpn_neck import BiFPNNeck
            self.bifpn = BiFPNNeck(channels=hidden_dim, num_levels=self.num_feature_levels, repeats=int(bifpn_repeats))
        else:
            self.bifpn = None
    ...
    def forward(self, samples: NestedTensor, targets:List=None):
        ...
        for l, feat in enumerate(features):
            ...
            proj = self.input_proj[l](src)
            # If projection changes spatial size (e.g. P2 stride=2), we must also align mask/pos.
            if mask is not None and tuple(mask.shape[-2:]) != tuple(proj.shape[-2:]):
                mask = F.interpolate(mask[None].float(), size=proj.shape[-2:]).to(torch.bool)[0]
            if l < len(poss) and poss[l] is not None and tuple(poss[l].shape[-2:]) != tuple(proj.shape[-2:]):
                poss[l] = F.interpolate(poss[l], size=proj.shape[-2:], mode="bilinear", align_corners=False)
            srcs.append(proj)
            masks.append(mask)
...
class SetCriterion(nn.Module):
    ...
    def __init__(
        ...
        *,
        ...
        Parameters:
            ...
            box_iou_loss_type: 'giou' or 'siou' for bbox regression IoU term (keys remain loss_giou for compatibility).
        """
        super().__init__()
        self.num_classes = num_classes
        ...
        assert box_iou_loss_type in ("giou", "siou"), box_iou_loss_type
        self.box_iou_loss_type = box_iou_loss_type
        self.use_class_weights = bool(use_class_weights) and (class_weights is not None)
        if self.use_class_weights:
            cw = class_weights
            if not isinstance(cw, torch.Tensor):
                cw = torch.as_tensor(cw, dtype=torch.float32)
            if cw.numel() != int(num_classes):
                raise ValueError(f"class_weights must have length {num_classes}, got {cw.numel()}")
    ...
    def loss_labels(self, outputs, targets, indices, num_boxes, log=True):
        ...
        if self.use_class_weights:
            loss_ce = sigmoid_focal_loss_with_class_weights(
                src_logits,
    ...
    def loss_boxes(self, outputs, targets, indices, num_boxes):
        ...
        if self.box_iou_loss_type == "giou":
            loss_giou = 1 - torch.diag(box_ops.generalized_box_iou(src_xyxy, tgt_xyxy))
        else:
            loss_giou = torch.diag(box_ops.siou_loss_pairwise_xyxy(src_xyxy, tgt_xyxy))
...
def build_dino(args):
    ...
    model = DINO(
        ...
        nheads=args.nheads,
        # structural upgrades
        p2_proj_stride=int(getattr(args, "p2_proj_stride", 1) or 1),
        use_bifpn_neck=bool(getattr(args, "use_bifpn_neck", False)),
        bifpn_repeats=int(getattr(args, "bifpn_repeats", 2) or 2),
    ...
    criterion = SetCriterion(
        ...
        losses=losses,
        box_iou_loss_type=getattr(args, "box_iou_loss_type", "giou"),
        class_weights=getattr(args, "visdrone_class_weights", None),
        use_class_weights=bool(getattr(args, "use_visdrone_class_weights", False)),
    )
    criterion.to(device)
