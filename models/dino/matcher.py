...
from util.box_ops import box_cxcywh_to_xyxy, generalized_box_iou, siou_loss_pairwise_xyxy
...
class HungarianMatcher(nn.Module):
    ...
    def __init__(
        ...
        Params:
            ...
            iou_loss_type: 'giou' (minimize -GIoU) or 'siou' (minimize SIoU loss)
        """
        super().__init__()
        ...
        assert iou_loss_type in ("giou", "siou"), iou_loss_type
    ...
    def forward(self, outputs, targets):
        ...
        if self.iou_loss_type == "giou":
            cost_iou = -generalized_box_iou(pred_xyxy, tgt_xyxy)
        else:
            cost_iou = siou_loss_pairwise_xyxy(pred_xyxy, tgt_xyxy)

        # Final cost matrix
        C = self.cost_bbox * cost_bbox + self.cost_class * cost_class + self.cost_giou * cost_iou
...
def build_matcher(args):
    ...
    iou_loss_type = getattr(args, "box_iou_loss_type", "giou")
    if args.matcher_type == 'HungarianMatcher':
        return HungarianMatcher(
            ...
            focal_alpha=args.focal_alpha, iou_loss_type=iou_loss_type,
        )
